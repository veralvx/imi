...
///
/// `Debug` but deliberately not `Clone`: like `Target`, this is a token
/// threaded from the phase that produces it (Phase 1) to the phase that
/// consumes it (Phase 2), not a value a caller has reason to duplicate.
/// Adding `Clone` later is a non-breaking change; removing one is not,
/// so the derive waits for a use case.
#[derive(Debug)]
pub struct TargetDevts {
...
/// Parse the `major:minor` line sysfs writes into a `dev` file.
///
/// Extracted so the malformed cases are reachable: the reader around it
/// needs a real sysfs entry, and a devt silently parsed as `(0, 0)`
/// would compare equal to every other zero devt in the target set —
/// exactly the trap the `S_IFBLK` gate elsewhere exists to close.
///
/// # Errors
///
/// Returns an error when the line has no `:`, or when either side is not
/// a base-ten integer.
fn parse_devt(raw: &str, path: &str) -> Result<(u64, u64)> {
    let s = raw.trim();
    let (maj, min) = s.split_once(':').ok_or_else(|| err!("malformed {path}: {s:?}"))?;
    let maj: u64 = maj.parse().with_context(|| format!("parse major in {path}"))?;
    let min: u64 = min.parse().with_context(|| format!("parse minor in {path}"))?;
    Ok((maj, min))
}
...
pub(crate) fn block_rdev_of(path: &Path) -> Option<(u64, u64)> {
    ...
    // Mirrors the `S_IFMT`/`S_IFBLK` test in
    // `phases::phase_0::ensure_block_device`; see the note there for why
    // the two are kept separate. Change both together.
...
pub(crate) fn disable_swaps_on_target<E: Events + ?Sized>(
    targets: &TargetDevts,
    events: &mut E,
) -> Result<()> {
    let text = match fs::read_to_string("/proc/swaps") {
        ...
        Err(e) => return Err(err!("read /proc/swaps: {e}")),
    };
    ...
    for line in lines {
        ...
        events.action(&format!("disabling swap on {unescaped}"));
...
/// Refuse while anything is still mounted.
///
/// Extracted from both Phase 1 and Phase 2 because the negation is the
/// safety property: inverted, the pipeline proceeds precisely when
/// mounts remain — flashing a device the kernel is still writing to.
/// Neither caller is reachable without a real device, so the decision
/// has to be testable on its own.
///
/// Lives in `common` rather than in either phase because both need it,
/// and the crate's rule — stated in `common::context` — is that a type
/// or function named by two phases is shared, so phases never import
/// from each other. It used to sit in Phase 2 with Phase 1 reaching
/// across for it.
///
/// # Errors
///
/// Returns an error naming the count when `mounts` is non-empty.
pub(crate) fn ensure_unmounted(mounts: &[MountInfo], what: &str) -> Result<()> {
    if !mounts.is_empty() {
        bail!("{what}; {} mount(s) remain; aborting", mounts.len());
    }
    Ok(())
}

/// Order mounts so a child is always unmounted before its parent.
///
/// `umount2` on a directory that still has something mounted beneath it
/// returns `EBUSY`, and this tool treats `EBUSY` as "stop and tell the
/// operator" — so getting the order wrong turns a perfectly unmountable
/// stick into a refusal with a message about open files that names the
/// wrong cause.
///
/// Sorting by path length descending is sufficient, not merely
/// convenient: a parent's path is a strict prefix of its child's, so it
/// is strictly shorter. Two mounts of equal length cannot be nested in
/// each other, and their relative order is therefore free.
///
/// Extracted from [`unmount_all`] so the ordering can be tested without
/// a mounted filesystem — the caller needs real mounts, this does not.
fn deepest_first(mounts: &[MountInfo]) -> Vec<&MountInfo> {
...
/// # Errors
///
/// Returns the first `umount2` failure, naming the mount.
pub(crate) fn unmount_all<E: Events + ?Sized>(mounts: &[MountInfo], events: &mut E) -> Result<()> {
    for m in &deepest_first(mounts) {
...
mod tests {
    ...
    /// The mount check must refuse when mounts remain, not when they are
    /// gone.
    ///
    /// Inverted, the pipeline proceeds precisely when the device is
    /// still mounted — writing under a live filesystem. Both Phase 1 and
    /// Phase 2 route through this, and neither is reachable without a
    /// real device, which is why the decision is tested here rather than
    /// through either caller.
    #[test]
    fn unmounted_check_refuses_only_when_mounts_remain() {
        let mk = |t: &str| MountInfo { devt: (8, 1), target: PathBuf::from(t), source: None };

        ensure_unmounted(&[], "nothing mounted").expect("an empty list must pass");

        let one = vec![mk("/mnt/a")];
        let err = ensure_unmounted(&one, "target still mounted").unwrap_err();
        assert!(err.to_string().contains("1 mount(s) remain"), "{err}");

        let two = vec![mk("/mnt/a"), mk("/mnt/b")];
        assert!(ensure_unmounted(&two, "x").unwrap_err().to_string().contains("2 mount(s)"));
    }

    /// A child must never be unmounted after its parent.
    ///
    /// The ordering was two lines inside `unmount_all`, which needs real
    /// mounts and so is unreachable from a unit test — meaning the one
    /// invariant that decides whether a valid stick unmounts cleanly or
    /// gets refused with a misleading message was covered by nothing.
    #[test]
    fn deepest_first_never_puts_a_parent_before_its_child() {
        let mk = |t: &str| MountInfo {
            devt: (8, 17),
            target: PathBuf::from(t),
            source: Some(PathBuf::from("/dev/sdb1")),
        };
        // Deliberately shuffled, and including a long non-nested sibling
        // that a naive depth-by-component count would order differently.
        let mounts = vec![
            mk("/media"),
            mk("/media/a/b/c"),
            mk("/media/verylongsiblingname"),
            mk("/media/a"),
            mk("/media/a/b"),
        ];
        ...
        let pos =
            |t: &str| ordered.iter().position(|m| m.target == *Path::new(t)).expect("present");

        // Every ancestor/descendant pair, in both spellings.
        for (child, parent) in [
            ("/media/a/b/c", "/media/a/b"),
            ("/media/a/b", "/media/a"),
            ("/media/a", "/media"),
            ("/media/a/b/c", "/media"),
            ("/media/verylongsiblingname", "/media"),
        ] {
            assert!(pos(child) < pos(parent), "{child} must be unmounted before {parent}");
        }

        assert_eq!(ordered.len(), mounts.len(), "no mount may be dropped");
    }
    ...
    /// Only `0`-`7` are octal digits. `8` and `9` must make the escape
    /// invalid so the sequence passes through literally.
    ///
    /// The boundary matters beyond pedantry: with `8` wrongly accepted,
    /// `\\080` decodes to `0o100` = `@`, silently rewriting a mountinfo
    /// path. A mount point that no longer matches its real path is a
    /// mount this tool fails to notice.
    #[test]
    fn unescape_rejects_non_octal_digits() {
        // `8` in any position invalidates the whole escape.
        assert_eq!(unescape_proc_octal("\\080"), "\\080");
        assert_eq!(unescape_proc_octal("\\800"), "\\800");
        assert_eq!(unescape_proc_octal("\\008"), "\\008");
        assert_eq!(unescape_proc_octal("\\099"), "\\099");
        // Still correct either side of an invalid sequence.
        assert_eq!(unescape_proc_octal("/a\\080/b\\040c"), "/a\\080/b c");
    }
...
";
    ...
    /// `block_rdev_of` must return a devt **only** for block devices.
    ///
    /// This is the `S_IFBLK` gate. Without it, `stat` on a regular file
    /// reports `st_rdev == 0`, and a `(0, 0)` devt would compare equal to
    /// every other zero devt in the target set — the trap this check
    /// exists to close. Both directions are asserted because inverting
    /// the comparison, or turning the mask `&` into `|`, breaks exactly
    /// one of them.
    #[test]
    fn block_rdev_of_accepts_only_block_devices() {
        let dir = std::env::temp_dir();
        let file = dir.join(format!("imi-notblock-{}", std::process::id()));
        fs::write(&file, b"x").unwrap();

        assert_eq!(block_rdev_of(&file), None, "a regular file is not a block device");
        assert_eq!(block_rdev_of(&dir), None, "a directory is not a block device");
        assert_eq!(
            block_rdev_of(Path::new("/dev/null")),
            None,
            "a character device is not a block device"
        );
        assert_eq!(
            block_rdev_of(Path::new("/nonexistent/imi-probe")),
            None,
            "a missing path yields None rather than panicking"
        );

        // Only `is_some()` is asserted. An earlier version also checked
        // `major == 7`, which is a fact about Linux's device-number
        // allocation rather than about this function, and it made the
        // test environment-dependent: under Miri the `stat` shim reports
        // the node as a block device but leaves `st_rdev` zero, so the
        // assertion failed on a correct implementation. What this test
        // exists to pin is the `S_IFBLK` gate — that a block device
        // yields a devt and nothing else does.
        let loop0 = Path::new("/dev/loop0");
        if loop0.exists() {
            assert!(
                block_rdev_of(loop0).is_some(),
    ...
    ///
    /// A devt wrongly parsed as `(0, 0)` compares equal to every other
    /// zero devt in the target set, so the failure modes matter as much
    /// as the success one.
    #[test]
    fn devt_parsing_accepts_only_major_colon_minor() {
        assert_eq!(parse_devt("8:0\n", "/p").unwrap(), (8, 0));
        assert_eq!(parse_devt("  259:3  ", "/p").unwrap(), (259, 3), "whitespace is trimmed");
        assert_eq!(parse_devt("7:9", "/p").unwrap(), (7, 9));

        for bad in ["", "8", "8-0", ":0", "8:", "a:0", "8:b", "8:0:1", "-1:0"] {
            parse_devt(bad, "/p").unwrap_err();
        }

        // The message must name the file, since the caller cannot.
        let err = parse_devt("nope", "/sys/class/block/sdz/dev").unwrap_err();
        assert!(format!("{err:#}").contains("/sys/class/block/sdz/dev"), "{err:#}");
    }
}
