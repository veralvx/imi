...
/// small decoder reads.
const IMG_BUFREAD_CAP: usize = 2 * 1024 * 1024;
...
impl std::fmt::Display for Compression {
    /// Renders as [`Compression::label`] — `raw`, `gzip`, `xz`, `bzip2`
    /// or `zstd`.
    ///
    /// The label is what the operator sees on the confirmation banner,
    /// so it is the natural `Display`. `label` is kept because it hands
    /// back a `&'static str` without formatting machinery, which
    /// `Display` cannot.
...
pub(crate) fn detect_compression(path: &Path) -> Result<Compression> {
    ...
    let n = read_full_best_effort(&mut f, &mut buf)
        .with_context(|| format!("read magic bytes of {}", path.display()))?;
    ...
    if let Some(kind) = unsupported_archive(prefix) {
        bail!(
            "{} is a {kind} archive, which this tool cannot decompress. Writing it \
             verbatim would produce a device that is not bootable, and verification \
             would pass because it re-reads the same bytes. Extract the disk image \
             first, then flash that.",
            path.display()
        );
    }
    Ok(classify(prefix))
...
/// Name the archive format if `prefix` is a compressed container this
/// crate cannot decode.
///
/// Detection is not decoration here. An unrecognised magic classifies as
/// [`Compression::Raw`], the archive is written to the device verbatim,
/// and Phase 5 re-reads it through the same raw path — so the comparison
/// passes and the operator is told SUCCESS while holding a device that
/// contains a `.zip` instead of a filesystem. The failure is silent in
/// both directions, which is the worst shape a failure can have here.
///
/// Only unambiguous magics are listed. A real disk image begins with a
/// boot sector — a jump instruction, `EB`/`FA`/`33` — so none of these
/// can collide with one. `.lzma` (`5D 00 00`) is deliberately absent:
/// three bytes, two of them zero, is not distinctive enough to refuse a
/// flash over.
fn unsupported_archive(prefix: &[u8]) -> Option<&'static str> {
    const KNOWN: &[(&[u8], &str)] = &[
    ...
    KNOWN.iter().find(|(magic, _)| prefix.starts_with(magic)).map(|(_, name)| *name)
}
...
mod tests {
    ...
    /// Archives this crate cannot decode must be named, not classified
    /// as raw.
    ///
    /// The consequence of missing one is not a decode failure: it is a
    /// verbatim write of the archive to the device, a verification pass
    /// (Phase 5 re-reads through the same raw path), and a SUCCESS line
    /// over a device that will not boot.
    #[test]
    fn unsupported_archives_are_recognised() {
        for (bytes, expect) in [
            (&[0x50, 0x4B, 0x03, 0x04][..], "zip"),
            (&[0x50, 0x4B, 0x05, 0x06][..], "zip (empty archive)"),
            (&[0x37, 0x7A, 0xBC, 0xAF, 0x27, 0x1C][..], "7-zip"),
            ...
            assert_eq!(unsupported_archive(bytes), Some(expect), "{bytes:02X?}");
        }
    }

    /// The formats this crate *does* handle, and real images, must not
    /// be caught by the archive check.
    #[test]
    fn supported_formats_and_disk_images_are_not_refused() {
        for ok in [
            &[0x1F, 0x8B][..],                         // gzip
            &[0xFD, 0x37, 0x7A, 0x58, 0x5A, 0x00][..], // xz
            &[0x42, 0x5A, 0x68][..],                   // bzip2
            &[0x28, 0xB5, 0x2F, 0xFD][..],             // zstd
            &[0xEB, 0x3C, 0x90][..],                   // FAT boot sector
            &[0xFA, 0x33, 0xC0][..],                   // typical MBR
            &[0x33, 0xED, 0x90][..],                   // Linux boot sector
            &[0x4C, 0x55, 0x4B, 0x53, 0xBA, 0xBE][..], // LUKS1/2 header
            &[0x00; 8][..],                            // zeroed device / ISO pad
            &[0x50, 0x4B][..],                         // shorter than any magic
            &[][..],                                   // empty file
        ] {
            assert_eq!(unsupported_archive(ok), None, "{ok:02X?}");
        }
    }

    /// `7z`'s magic starts `37 7A`, and xz's starts `FD 37`. Neither is
    /// a prefix of the other, but the digits invite a transposition.
    #[test]
    fn seven_zip_and_xz_are_not_confused() {
        assert_eq!(unsupported_archive(&[0x37, 0x7A, 0xBC, 0xAF, 0x27, 0x1C]), Some("7-zip"));
        assert_eq!(unsupported_archive(&[0xFD, 0x37, 0x7A, 0x58, 0x5A, 0x00]), None);
        assert_eq!(classify(&[0xFD, 0x37, 0x7A, 0x58, 0x5A, 0x00]), Compression::Xz);
        assert_eq!(classify(&[0x37, 0x7A, 0xBC, 0xAF, 0x27, 0x1C]), Compression::Raw);
    }
    ...
    /// A reader that yields `Interrupted` before every real read, to
    /// exercise the EINTR retry arm.
    struct InterruptOnce {
        /// Remaining interruptions to inject before data flows.
        interrupts: usize,
        /// Bytes handed out once interruptions are exhausted.
        data: Vec<u8>,
        /// How much of `data` has been delivered.
        pos: usize,
    }
    ...
    impl Read for InterruptOnce {
        fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
            if self.interrupts > 0 {
                self.interrupts = self.interrupts.saturating_sub(1);
            ...
            buf.get_mut(..n)
                .unwrap_or_default()
                .copy_from_slice(remaining.get(..n).unwrap_or_default());
            self.pos = self.pos.saturating_add(n);
            Ok(n)
        }
    }

    /// `Interrupted` must be retried transparently, not surfaced and not
    /// treated as EOF. A signal arriving mid-read would otherwise cause
    /// a short magic-byte read and a misdetected compression format.
    #[test]
    fn read_full_best_effort_retries_on_interrupted() {
        let mut r = InterruptOnce { interrupts: 3, data: vec![0x1F, 0x8B, 0x08, 0x00], pos: 0 };
        let mut buf = [0_u8; 4];
        let n = read_full_best_effort(&mut r, &mut buf).unwrap();
        assert_eq!(n, 4, "all bytes must arrive despite the interruptions");
        assert_eq!(buf, [0x1F, 0x8B, 0x08, 0x00]);
    }

    /// Any other error kind must propagate rather than being retried
    /// forever — the inverse of the case above.
    #[test]
    fn read_full_best_effort_propagates_non_interrupted_errors() {
        struct AlwaysBroken;
        ...
        let mut buf = [0_u8; 4];
        let err = read_full_best_effort(&mut AlwaysBroken, &mut buf).unwrap_err();
        assert_eq!(err.kind(), io::ErrorKind::BrokenPipe);
    }

    /// The read-ahead buffer is 2 MiB. Pinned because the value is an
    /// arithmetic expression: a slip turning `*` into `+` or `/` would
    /// silently shrink it to a few kilobytes and cost a syscall per
    /// decoder read.
    #[test]
    fn img_bufread_cap_is_two_mebibytes() {
        assert_eq!(IMG_BUFREAD_CAP, 0x0020_0000);
    }

    /// `Display` must render exactly what `label` returns.
    ///
    /// Two ways to name the same thing; if they drift, an operator sees
    /// one string on the banner and a different one in a log line.
    #[test]
