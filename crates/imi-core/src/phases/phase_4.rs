...
//!
//! [`run`] sets the guard phase, opens the image, and drives `flash`.
//!
...
//!
//! - UX: this phase renders nothing. It emits `progress(0, raw_size)`
//!   immediately before the write loop — after decompressor init and
//!   buffer allocation, so that setup cost cannot contaminate the rate
//!   a front end computes — then `progress(written, raw_size)` per
//!   chunk. The `imi` binary turns those into an `indicatif` bar; see
//!   `crates/imi/src/progress.rs`.
...
/// Write the whole image to the claimed device.
///
/// Phases are ordered: this one assumes every earlier phase succeeded.
///
/// # Errors
///
/// Returns an error if the image cannot be opened or decompressed, if
/// the decompressed stream would pass the end of the device, if a write
/// fails, or if the run is cancelled.
///
/// # Panics
///
...
pub fn run<E: Events + ?Sized>(
    ...
    // Every failure reports `Indeterminate`, including the two
    // precondition checks that run before a single byte is written.
    //
    // That is deliberate rather than sloppy. Reaching Phase 4 with the
    // wrong device or a guard Phase 3 never armed means the caller drove
    // the pipeline out of order, and a crate whose preconditions have
    // been violated cannot vouch for what else was done to the device.
...
fn flash_serial<E: Events + ?Sized>(
    ...
    set_direct(guard.file(), true).context("enabling O_DIRECT for flash phase")?;

    // Critical: open the front end's bar and start its clock here, after
    // setup and immediately before the write loop. Without this the bar
    // is not created until the first chunk completes, so its opening
    // frame is missed entirely, and construction plus first-chunk fill
    // time count toward the rate — a large initial spike, especially
    // visible with --throttle, where the first chunk writes at
    // unthrottled speed before any sleep has fired.
    events.progress(UiPhase::Flash, 0, raw_size);
    ...
    loop {
        ...
        // threading plan. On a helper error the phase boundary reports
        // `PhaseOutcome::Failed`, which is what tells a front end to
        // leave its last rendered state on screen as the abort record.
        ...
        events.progress(UiPhase::Flash, total, raw_size);
...
/// propagates the typed error unchanged. Reporting is the caller's job:
/// emit `progress` after a successful return; the phase boundary
/// reports failure.
///
...
fn process_chunk(
    ...
    set_direct(guard.file(), false).context("disabling O_DIRECT for tail write")?;
    let tail = buf
        .as_slice()
        .get(..filled)
...
/// Shared post-loop work for the flash arms: clear `O_DIRECT`
/// (hardening) and issue the durable `fdatasync`.
///
...
fn flash_finalize(guard: &ArmedGuard) -> Result<()> {
    ...
    set_direct(guard.file(), false).context("clearing O_DIRECT after flash write loop")?;
...
#[expect(
    ...
    reason = "the dispatcher threads the phase's inputs plus the events sink; \
...
pub(crate) fn flash<E: Events + ?Sized>(
    ...
    if comp.is_compressed() {
        ...
        // total unknown), so the pipelined arm reports `None` as its
        // total and a front end shows an indeterminate indicator.
        flash_pipelined(guard, reader, dev_size, throttle, cancel, events)
    } else {
        flash_serial(guard, reader, raw_size, dev_size, throttle, cancel, events)
    }
}
...
fn worker_loop<R: Read>(
    ...
    loop {
        ...
        let filled = match fill_buffer(&mut reader, buf.as_mut_slice()) {
            ...
            Err(e) => {
                let wrapped =
                    Err(crate::error::Error::from_source(e).context("reading from image stream"));
...
fn flash_pipelined<R: Read + Send + 'static, E: Events + ?Sized>(
    ...
    // Compressed input: the decompressed total is unknown, so the front
    // end gets `None` and shows a spinner. Emitted here, immediately
    // after setup, so the bar exists for the whole first chunk — the
    // same point the bar used to be constructed.
    events.progress(UiPhase::Flash, 0, None);
    ...
    'write_loop: loop {
        ...
        match process_chunk(guard, &buf, filled, offset, dev_size) {
            Ok(ChunkOutcome::Continue { end }) => {
                offset = end;
                total = end;
                events.progress(UiPhase::Flash, total, None);
...
fn write_direct(guard: &ArmedGuard, data: &[u8], offset: u64) -> Result<()> {
    match guard.file().write_all_at(data, offset) {
        ...
        Err(e) => Err(crate::error::Error::from_source(e)
...
mod tests {
    ...
    /// Real guard over a fresh tempfile, plus a path that unlinks itself.
    ///
    ...
    fn flash_finalize_clears_o_direct_and_syncs() {
        ...
        // Set the flag so clearing it is observable. Asserting the
        // effect, not just that the call returns Ok: the previous
        // version of this test checked a progress bar had finished,
        // and when the bar moved to the binary that check went with it
        // — leaving `flash_finalize` replaceable by `Ok(())`, which
        // would skip the durable fdatasync entirely.
        set_direct(guard.file(), true).expect("set O_DIRECT on a tempfile");
        let before = nix::fcntl::fcntl(guard.file(), nix::fcntl::F_GETFL).unwrap();
        assert!(before & libc::O_DIRECT != 0, "precondition: O_DIRECT is set");

        flash_finalize(&guard).unwrap();
    ...
    /// A short fill is the in-band EOF signal, and the worker must act
    /// on it by exiting — while the pool still has buffers and main is
    /// still listening.
    ///
    /// The other worker tests all end the run by closing a channel from
    /// main, so they pass even if the worker never recognises EOF at
    /// all. This one keeps both channels open: the only way the second
    /// `recv` can fail is the worker having returned on its own.
    #[test]
    fn worker_exits_on_eof_without_main_closing_channels() {
        let (filled_tx, filled_rx) = mpsc::channel::<FilledItem>();
        let (free_tx, free_rx) = mpsc::channel::<AlignedBuf>();
        // Two buffers: if the worker wrongly loops after EOF it can get
        // another one and send a spurious empty chunk.
        for _ in 0..2_u8 {
            free_tx.send(AlignedBuf::new().unwrap()).unwrap();
        }
        let mirror = Arc::new(AtomicBool::new(false));
        ...
        let short = Cursor::new(vec![0xAB_u8; 100]);
        let h = thread::spawn(move || worker_loop(short, &m, &filled_tx, &free_rx));

        let (_buf, n) = filled_rx.recv().unwrap().unwrap();
        assert_eq!(n, 100, "the whole short input arrives as one chunk");

        // `free_tx` is deliberately still alive here.
        assert!(
            filled_rx.recv().is_err(),
            "worker must exit on the short fill, not send another chunk"
        );
        h.join().unwrap();
        drop(free_tx);
    }
    ...
    fn pipelined_propagates_worker_error_with_context() {
        ...
        let err = flash_pipelined(&mut guard, reader, BUF_SIZE as u64, None, &cancel, &mut ())
            .unwrap_err();
    ...
    fn pipelined_resumes_worker_panic_on_main_thread() {
        ...
        let caught = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            flash_pipelined(&mut guard, PanickingReader, BUF_SIZE as u64, None, &cancel, &mut ())
        }));
    ...
    fn arms_produce_identical_bytes_for_identical_input() {
        ...
        let o1 =
            flash_serial(&mut g1, r1, Some(pattern.len() as u64), dev_size, None, &cancel, &mut ())
                .unwrap();

        let (mut g2, d2) = tempfile_guard("par-pipe");
        let r2 = ImageReader::open(&gz_p, Compression::Gzip).unwrap();
        let o2 = flash_pipelined(&mut g2, r2, dev_size, None, &cancel, &mut ()).unwrap();
        ...
        // d1 and d2 are `TempPath` and unlink themselves; these two are
        // plain paths this test created.
        for p in [raw_p, gz_p] {
    ...
    /// A non-capacity write failure must not be reported as a full
    /// device.
    ///
    /// The guard `is_capacity_error(&e)` decides between two very
    /// different messages. Replaced with `true`, every write failure
    /// would tell the operator the image is larger than the device —
    /// sending them to buy a bigger stick when the real fault is a dying
    /// one, or a revoked permission. The predicate itself is tested
    /// separately; this pins the branch that consults it.
    ///
    ...
    fn a_non_capacity_write_error_keeps_its_own_diagnostic() {
        ...
        let text = format!("{err:#}");
        assert!(
            !text.contains("larger than the target device"),
            "a permission failure must not be reported as a full device: {text}"
        );
        assert!(text.contains("writing 4 MiB chunk"), "{text}");

        let tail_err = write_tail(&guard, &[0_u8; 8], 0).unwrap_err();
        let tail_text = format!("{tail_err:#}");
        assert!(
            !tail_text.contains("larger than the target device"),
            "the tail path must not either: {tail_text}"
        );
